...
:list_workroom: - ``атрибут класса`` список отделов даной студии (объекты). Заполняется привыполнеии метода *get_list()*, значение по умолчанию - *False*.

:dict_by_name: - ``атрибут класса`` словарь отделов даной студии (объекты) с ключами по именам. Заполняется привыполнеии метода *get_list()*, значение по умолчанию - *False*.

:dict_by_id: - ``атрибут класса`` словарь отделов даной студии (объекты) с ключами по *id*. Заполняется привыполнеии метода *get_list()*, значение по умолчанию - *False*.
...
.. py:function:: get_list([return_type = False, objects=True])
  ...
  .. note:: Заполняет ``атрибуты класса``: **list_workroom**, **dict_by_name**, **dict_by_id**.
