...
.. py:function:: get_work_final_file_path()

  .. rubric:: Студийный работник:
  
  * загрузка списко *commit + pull* данного артиста.
  * путь по последней записи.
  
  .. rubric:: Аутсорс:
  
  * загрузка push + commit списков без учёта пользователя.
  * если последняя запись commit - то берём по этой версии.
      * если файл этой версии не в ворк директории данного пользователя, то будет предложено сделать push.
  * если последняя запись push - и commit версия этого push находится в work директории данного пользователя - то эта commit версия, иначе - push.
